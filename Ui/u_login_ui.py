# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file 'd:\POO\PROYECTOS\Proyectoe\Ui\u_login.ui'
#
# Created by: PyQt5 UI code generator 5.15.9
#
# WARNING: Any manual changes made to this file will be lost when pyuic5 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt5 import QtCore, QtGui, QtWidgets


class Ui_Dialog(object):
    def setupUi(self, Dialog):
        Dialog.setObjectName("Dialog")
        Dialog.resize(400, 500)
        Dialog.setLayoutDirection(QtCore.Qt.LeftToRight)
        Dialog.setStyleSheet("background-color:rgb(54, 54, 54)")
        self.Title = QtWidgets.QLabel(Dialog)
        self.Title.setGeometry(QtCore.QRect(0, 0, 400, 50))
        font = QtGui.QFont()
        font.setFamily("Segoe UI Symbol")
        font.setPointSize(20)
        self.Title.setFont(font)
        self.Title.setStyleSheet("background-color: rgb(255, 255, 255)")
        self.Title.setTextFormat(QtCore.Qt.AutoText)
        self.Title.setAlignment(QtCore.Qt.AlignCenter)
        self.Title.setWordWrap(False)
        self.Title.setObjectName("Title")
        self.label = QtWidgets.QLabel(Dialog)
        self.label.setGeometry(QtCore.QRect(30, 150, 91, 32))
        font = QtGui.QFont()
        font.setFamily("Segoe UI Symbol")
        font.setPointSize(13)
        self.label.setFont(font)
        self.label.setLayoutDirection(QtCore.Qt.LeftToRight)
        self.label.setStyleSheet("background-color:rgb(255, 255, 255);\n"
"border: 2px Solid rgb(122, 143, 168);")
        self.label.setAlignment(QtCore.Qt.AlignCenter)
        self.label.setObjectName("label")
        self.id_emp = QtWidgets.QLineEdit(Dialog)
        self.id_emp.setGeometry(QtCore.QRect(130, 150, 241, 32))
        font = QtGui.QFont()
        font.setFamily("Segoe UI Symbol")
        font.setPointSize(13)
        self.id_emp.setFont(font)
        self.id_emp.setStyleSheet("background-color:rgb(255, 255, 255);\n"
"border: 2px solid rgb(122, 143, 168);\n"
"padding: 5px;")
        self.id_emp.setText("")
        self.id_emp.setFrame(True)
        self.id_emp.setObjectName("id_emp")
        self.label_2 = QtWidgets.QLabel(Dialog)
        self.label_2.setGeometry(QtCore.QRect(29, 210, 91, 32))
        font = QtGui.QFont()
        font.setFamily("Segoe UI Symbol")
        font.setPointSize(13)
        self.label_2.setFont(font)
        self.label_2.setLayoutDirection(QtCore.Qt.LeftToRight)
        self.label_2.setStyleSheet("background-color:rgb(255, 255, 255);\n"
"border: 2px Solid rgb(122, 143, 168);")
        self.label_2.setAlignment(QtCore.Qt.AlignCenter)
        self.label_2.setObjectName("label_2")
        self.pwd_emp = QtWidgets.QLineEdit(Dialog)
        self.pwd_emp.setGeometry(QtCore.QRect(130, 210, 241, 32))
        font = QtGui.QFont()
        font.setFamily("Segoe UI Symbol")
        font.setPointSize(13)
        self.pwd_emp.setFont(font)
        self.pwd_emp.setStyleSheet("background-color:rgb(255, 255, 255);\n"
"border: 2px solid rgb(122, 143, 168);\n"
"padding: 5px;")
        self.pwd_emp.setText("")
        self.pwd_emp.setFrame(True)
        self.pwd_emp.setObjectName("pwd_emp")
        self.pushlogin = QtWidgets.QPushButton(Dialog)
        self.pushlogin.setGeometry(QtCore.QRect(290, 260, 75, 30))
        font = QtGui.QFont()
        font.setFamily("Segoe UI Symbol")
        font.setPointSize(13)
        self.pushlogin.setFont(font)
        self.pushlogin.setStyleSheet("background-color: rgb(255, 255, 255);\n"
"border: 2px solid rgb(122, 143, 168);")
        self.pushlogin.setObjectName("pushlogin")

        self.retranslateUi(Dialog)
        QtCore.QMetaObject.connectSlotsByName(Dialog)

    def retranslateUi(self, Dialog):
        _translate = QtCore.QCoreApplication.translate
        Dialog.setWindowTitle(_translate("Dialog", "Dialog"))
        self.Title.setText(_translate("Dialog", "LOGIN"))
        self.label.setText(_translate("Dialog", "Id Emp"))
        self.label_2.setText(_translate("Dialog", "Password"))
        self.pushlogin.setText(_translate("Dialog", "Login"))
